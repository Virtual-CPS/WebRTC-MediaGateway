...
void MediaCapture::openStream(const std::string& filename, AVInputFormat* inputFormat, AVDictionary** formatParams)
{
	...
	// bfish: add, 2017.07.30
	int nRet;
	if (filename.find("rtsp://") != std::string::npos ||
		filename.find("RTSP://") != std::string::npos)
	{
		av_register_all();
		avdevice_register_all();
		nRet = avformat_network_init();
		if(nRet < 0)
			throw std::runtime_error("avformat_network_init() error: " + filename);

		AVDictionary *format_opts = nullptr;
		//av_dict_set(&format_opts, "max_delay", "5000000", 0);   // 5 seconds
		//av_dict_set(&format_opts, "rtsp_transport", "udp", 0);
		av_dict_set(&format_opts, "scan_all_pmts", "1", AV_DICT_DONT_OVERWRITE);
		av_dict_set(&format_opts, "rtsp_transport", "udp", 0);

		nRet = avformat_open_input(&_formatCtx, filename.c_str(), inputFormat, &format_opts);

		//av_dict_set(&format_opts, "scan_all_pmts", NULL, AV_DICT_MATCH_CASE);
		av_dict_free(&format_opts);
	}
	else
		nRet = avformat_open_input(&_formatCtx, filename.c_str(), inputFormat, formatParams);

	char szBuf[256];
	if (nRet < 0)
	{
		av_strerror(nRet, szBuf, sizeof(szBuf));
		std::string sErr(szBuf);
